...
pub fn add(state:&Arc<AppState>,path:&str)->Result<(),AppError>{
    ...
    let root=if selected.is_file(){selected.parent().ok_or(AppError::NotFound)?.to_path_buf()}else{selected};
    ...
    for(_id,existing,_)in state.db.library_paths()?{
        if canonical.starts_with(&existing)&&canonical!=existing{
            return Err(AppError::Message(format!("「{}」已包含在已有书库中，无需重复导入",canonical.file_name().and_then(|s|s.to_str()).unwrap_or("该文件夹"))));
        }
        if existing.starts_with(&canonical)&&existing!=canonical{
            return Err(AppError::Message(format!("「{}」包含已有书库，请直接导入其上级文件夹",canonical.file_name().and_then(|s|s.to_str()).unwrap_or("该文件夹"))));
        }
    }
...
pub fn start_watchers(app:&AppHandle,state:&Arc<AppState>)->Result<(),AppError>{
    let mut list=state.watchers.lock();list.clear();
    for(_,root,_)in state.db.library_paths()?{
        ...
        let scanning=Arc::new(AtomicBool::new(false));let pending=Arc::new(AtomicBool::new(false));
        ...
        let mut watcher=notify::recommended_watcher(move|event:Result<notify::Event,notify::Error>|{
            ...
            if scanning_cb.swap(true,Ordering::Relaxed){return;}
            let app=app.clone();let state=state.clone();let s=scanning_cb.clone();let p=pending_cb.clone();
            std::thread::spawn(move||{
                loop{
                    ...
                    if !p.load(Ordering::Relaxed){s.store(false,Ordering::Relaxed);break;}
                }
            });
        }).map_err(|e|AppError::Message(e.to_string()))?;
        watcher.watch(&root,RecursiveMode::Recursive).map_err(|e|AppError::Message(e.to_string()))?;
...
fn scan_changed(state:&Arc<AppState>,changed:&Path)->Result<(),AppError>{
    ...
    for(id,root,_)in state.db.library_paths()?{
        if changed.starts_with(&root)&&matched.as_ref().map_or(true,|(_,r)|root.starts_with(r)){matched=Some((id,root));}
    }
    match matched{Some((id,root))=>scan(state,&id,&root),None=>refresh_all(state)}
}

pub fn create_entry(state:&Arc<AppState>,i:FileCreateInput)->Result<(),AppError>{let(_,root,_)=find_library(state,&i.library_id)?;let parent=if i.parent_path.is_empty(){root.clone()}else{safe_path(&root,&i.parent_path)?};let name=clean_name(&i.name)?;let path=parent.join(if i.kind=="file"&&!name.to_lowercase().ends_with(".txt"){format!("{}.txt",name)}else{name});if path.exists(){return Err(AppError::AlreadyExists);}if i.kind=="folder"{fs::create_dir(&path)?;}else{fs::write(&path,b"")?;}scan(state,&i.library_id,&root)}
...
fn clean_name(name:&str)->Result<String,AppError>{let n=name.trim();if n.is_empty()||n=="."||n==".."||n.contains('/')||n.contains('\\')||n.contains(':'){return Err(AppError::Message("名称包含无效字符".into()));}Ok(n.into())}

...
use std::{fs::{self,File},io::{Read,Write},path::{Path,PathBuf},sync::Arc};
use uuid::Uuid;
...
pub fn preview(state:&Arc<AppState>,id:&str,paragraphs:usize,chars_per_para:usize)->Result<Vec<String>,AppError>{
    ...
    let text=if stored.summary.format=="epub"{
        epub::preview_text(&path,paragraphs.saturating_mul(chars_per_para).saturating_mul(4))?
    }else{
...
pub fn read(state:&Arc<AppState>,id:&str)->Result<DocumentContent,AppError>{
    ...
    let (current_hash,changed)=if !stored.content_hash.is_empty()&&stored.summary.modified_at==file_mtime{
    ...
    let loaded=DocumentContent{summary,absolute_path:path.to_string_lossy().into_owned(),content,content_hash:current_hash,encoding,newline,editable,chapters:state.db.chapters(id)?,annotations:state.db.annotations(id)?,reading_progress:state.db.progress(id)?};
...
fn file_mtime(path:&Path)->i64{fs::metadata(path).ok().and_then(|meta|meta.modified().ok()).and_then(|time|time.duration_since(std::time::UNIX_EPOCH).ok()).map(|d|d.as_secs()as i64).unwrap_or(0)}

...
pub fn parse(path: &Path, document_id: &str) -> Result<ParsedEpub, AppError> {
    parse_limited(path, document_id, None)
}
...
fn parse_limited(
...
) -> Result<ParsedEpub, AppError> {
    let file = File::open(path)?;
    let mut zip = ZipArchive::new(file)?;
    if zip.by_name("META-INF/encryption.xml").is_ok() {
        return Err(AppError::Message("该 EPUB 已加密，熊猫阅读无法读取".into()));
    }
    let mut container = String::new();
    zip.by_name("META-INF/container.xml")?
        .read_to_string(&mut container)?;
    let doc =
        Document::parse(&container).map_err(|_| AppError::Message("EPUB 容器信息损坏".into()))?;
    let opf_path = doc
        .descendants()
        .find(|n| n.has_tag_name("rootfile"))
        .and_then(|n| n.attribute("full-path"))
        .ok_or_else(|| AppError::Message("EPUB 缺少内容索引".into()))?
        .to_string();
    let mut opf = String::new();
    zip.by_name(&opf_path)?.read_to_string(&mut opf)?;
    let opf_doc =
        Document::parse(&opf).map_err(|_| AppError::Message("EPUB 内容索引损坏".into()))?;
    let manifest: HashMap<String, String> = opf_doc
        ...
        .filter_map(|n| {
            Some((
                ...
                n.attribute("href")?.to_string(),
            ))
        })
        .collect();
    let spine: Vec<String> = opf_doc
        .descendants()
    ...
    let mut utf16_len = 0i64;
    for id in spine {
        if let Some(href) = manifest.get(&id) {
            let entry = normalize_zip(base.join(href));
            ...
            let heading = Selector::parse("h1,h2,h3,title").unwrap();
            let title = fragment
                .select(&heading)
                .next()
                .map(|e| e.text().collect::<String>().trim().to_string())
                .filter(|s| !s.is_empty())
                .unwrap_or_else(|| format!("章节 {}", chapters.len() + 1));
            let part = fragment
                .select(&body)
                .next()
            ...
            if !part.is_empty() {
                ...
                text.push_str(part);
                text.push_str("\n\n");
                utf16_len += (part.encode_utf16().count() + 2) as i64;
                if max_chapters.is_some_and(|limit| chapters.len() >= limit) {
                    break;
                }
            }
        }
    ...
    if text.is_empty() {
        return Err(AppError::Message("EPUB 没有可读取的正文".into()));
    }
    Ok(ParsedEpub { text, chapters })
}
fn normalize_zip(p: PathBuf) -> String {
    p.components()

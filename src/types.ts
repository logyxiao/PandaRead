export type Format = "txt" | "md" | "epub";
...
export interface DocumentSummary { id:string; libraryId:string; relativePath:string; title:string; format:Format; wordCount:number; modifiedAt:number; gender:string; genre:string; subgenre:string; lengthKind:string; purpose:Purpose; progress:Progress; shelf:""|"favorite"|"pending"|"dropped"; missing:boolean; tags:string[]; readOffset:number }
...
export interface ReaderSettings { theme:Theme; fontFamily:string; fontSize:number; letterSpacing:number; lineHeight:number; paperWidth:number; pageMargin:number; speechRate:number; brightness:number }
...
export interface RemoteDevice { deviceId:string; name:string; ip:string; lastSeen:number }
export interface RemoteStatus { running:boolean; port:number; lanUrls:string[]; publicAddress:string|null; tunnelState:"off"|"starting"|"on"|"error"; tunnelError:string|null; pairingCode:string; devices:RemoteDevice[] }
